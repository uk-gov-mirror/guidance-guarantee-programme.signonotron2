    ...
    context 'organisation admin' do
      ...
      it 'allows new batch upload for organisations within their organisation subtree' do
        expect(subject).to permit(organisation_admin,
                                  BatchInvitation.new(organisation_id: organisation_admin.organisation_id))
      end
      ...
      it 'blocks batch upload for organisations outside their organisation subtree' do
        ...
        expect(subject).not_to permit(create(:user_in_organisation, role: 'organisation_admin'),
                                      BatchInvitation.new(organisation_id: create(:organisation).id))
      end
    end
  end
end
