...
class NoisyBatchInvitationTest < ActionMailer::TestCase
  context "make_noise" do
    ...
    should "come from noreply-signon@" do
      assert_match /"?.* Signon development"? <noreply-pensionwise-signon-development@.*\.org\.uk>/, @email[:from].to_s
    end

    should "send to noreply-signon@" do
      assert_equal @email.to.count, 1
      assert_match /signon-pensionwise-alerts@.*\.org\.uk/, @email.to.first
    end
  ...
  context "make_noise when instance_name has been set" do
    ...
    should "from address should include the instance name" do
      assert_match /"?.* Signon Test Fools"? <noreply-pensionwise-signon-test-fools@.*\.org\.uk>/, @email[:from].to_s
    end
  end
  ...
  context "work correctly when no instance name is set" do
    ...
    should "from address should include the instance name" do
      assert_match /"?.* Signon"? <noreply-pensionwise-signon@.*\.org\.uk>/, @email[:from].to_s
    end
  end
end
